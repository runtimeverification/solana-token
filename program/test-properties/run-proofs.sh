...
ALL_NAMES=$(sed -n -e 's/^| \(test_[a-zA-Z0-9:_-]*\) *|.*/\1/p' proofs.md)
...
for name in ${TESTS}; do
    ...
    timeout --preserve-status -v ${TIMEOUT} \
            ...
            kmir prove-rs --smir artefacts/spl-token.smir.json \
    ...
    uv --project mir-semantics/kmir run -- \
        kmir show --proof-dir "${PROOF_DIR}" spl-token.smir.$start \
