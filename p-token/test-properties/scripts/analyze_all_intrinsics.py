...
def check_mir_semantics_support():
    ...
    k_source_dirs = [kdist.which(target) for target in ['mir-semantics.source', 'kompass.source']]
    k_source_files = [file for files in [source_dir.rglob('*.md') for source_dir in k_source_dirs] for file in files]

    print('K source files')
    print('--------------')
    for k_file in k_source_files:
        print(str(k_file))
